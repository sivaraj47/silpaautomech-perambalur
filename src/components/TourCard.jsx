function TourCard({ img, titles, description, isFullWidth,url}) {
  return (
   /*  <div className="group flex w-78 flex-col justify-center mt-7 bg-sky-100 rounded-xl border-2 border-transparent hover:border-cclor hover:shadow-strong transition ease-in m-2">
	...
	<div className={`group flex flex-col justify-center mt-7 bg-yellow-500/25 rounded-xl border-2 border-transparent hover:border-cclor hover:shadow-strong transition ease-in m-2 ${isFullWidth ? "w-full " : "w-78 max-md:w-full "}`} id="services">
	
	
	<img 	
	  src={img}
	  alt={titles}
	className={`rounded-xl  object-contain m-2 transform translate-x-0 group-hover:-translate-x-4 group-hover:shadow-xl1 transition ease 
	${isFullWidth ? "h-96 bg-top" : " h-48 max-md:h-96"}`} />
	
      <div className="p-2 font-roboto text-center">
        <a   onClick={() => window.open(url, "_blank")}
		 className="text-accent font-semibold text-[#222] bg-yellow-500 text-[14px] p-2  w-48 m-auto border-1 rounded-lg mb-2" href="#">{titles}</a>
        <p className="font-bold text-[14px] text-par mt-4">{description}</p>
      </div>
	</div>
